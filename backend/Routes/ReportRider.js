...
router.post("/reportrider", jwtAuth, async (req, result) => {
    ...
    try {
        console.log("Report Rider Request", req.body);
        const highestComplaintID = await Complaints.findOne().sort({ complaintID: -1 }).limit(1);
        const newComplaintID = highestComplaintID ? highestComplaintID.complaintID + 1 : 1;
